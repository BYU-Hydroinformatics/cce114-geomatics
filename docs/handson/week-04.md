...
> [!TIP]
> Two parts, about twenty-five minutes total. The field half needs you outdoors for five minutes.

**What you need.** A phone GPS app that shows five or more decimal places, such as **GPS Status**
or **GPS Test** on Android, or **GPS Coordinates** on iPhone. QGIS 3.44 LTR. No class data: you
will make your own small CSV, which is exactly what the class sheet becomes.

**Do this, in order.**

1. Walk outside and record two positions at two spots you can identify later, writing down every
   digit. Do it twice at the same spot so you have two readings to compare.
2. In a text editor make `practice_gps.csv` with a header line
   `name,site,lat,lon` and four rows of your readings. Western longitudes are negative.
3. QGIS: set the project CRS to **EPSG:26912** and add the Google satellite basemap.
4. **Layer > Add Layer > Add Delimited Text Layer...**, choose the CSV, **Point coordinates**,
   X = the longitude column, Y = the latitude column, Geometry CRS **EPSG:4326**. Add.
5. Right-click the layer > **Export > Save Features As...**, GeoPackage, CRS **EPSG:26912**. Add
   the saved file.
6. On the saved layer run the **Field Calculator** twice: decimal field `easting` as `$x`, then
   `northing` as `$y`. On campus expect eastings near 444,000 to 445,000 and northings near
   4,455,000 to 4,456,000.
7. Paste one reading into [tagis.dep.wv.gov/convert](https://tagis.dep.wv.gov/convert/) with output
   **UTM NAD83 Zone 12N** and confirm it matches to about a meter.
8. Use the **Measure Line** tool between your two readings of the same spot. That gap is the whole
   point of the session.

**You are ready when** your points land where you stood, the converter agrees with the field
calculator, and you can state your own repeat-reading error in meters.

**The one that bites.** Running `$x` on the imported CSV layer returns degrees, not meters.
`$x` is always in the layer's own CRS, so it must be run on the reprojected GeoPackage.
