# Week 11 Hands-On — Georeferencing in QGIS, and the Web Mapping with AI Kickoff

**Hands-On Practice**{ .badge .badge-handson } · *Day 21 · Thursday of [Week 11 — Geoplanning and Georeferencing](../weeks/week-11.md)*
...
> [!TIP]
> Thirty minutes, and do it twice if you can. Georeferencing is the least forgiving session of the
> semester because a bad set of control points is not obvious until the output loads.

**What you need.** QGIS 3.44 LTR, internet for the basemap, and a scanned map with no coordinates
in it. Download a historic USGS quad of Provo from
[USGS topoView](https://ngmdb.usgs.gov/topoview/) as a **JPEG**, not a GeoTIFF, so it genuinely has
no georeferencing. A campus plan sheet exported from PDF to PNG works too.

Check first that **Plugins > Manage and Install > Installed > Georeferencer GDAL** is ticked.

**Do this, in order.**

1. Add the Google satellite basemap and set the project CRS to **EPSG:26912**.
2. **Layer > Georeferencer...**, then **Open Raster** and choose your image.
3. With the **Add Point** tool, click a hard feature you can also find on the basemap, such as a
   road intersection or a bridge end. Click **From Map Canvas** and click the same feature on the
   main map. Repeat for four points spread to the corners, never along a line.
4. **Settings > Transformation Settings**: **Polynomial 1**, resampling **Cubic**, target CRS
   **EPSG:26912**, an output path next to the image, and **Load in project when done** ticked.
5. Read the **dX**, **dY**, **residual** columns and the **Mean error (RMS)** at the bottom. On a
   1:24,000 scan a 30 m residual is a misplaced click and 3 m is fine.
6. Untick one point and watch the RMS move. That is the quality-control moment of the session.
7. **Start Georeferencing**. Set the output to 50 percent opacity and toggle it. Check whether the
   roads line up across the whole image or only near your control points.
8. Add two more points in the worst area, switch to **Polynomial 2**, and rerun.
9. For the kickoff half: right-click any layer > **Export > Save Features As...**, GeoJSON, CRS
   **EPSG:4326**, then ask an AI assistant for a single Leaflet HTML file that loads it, and serve
   the folder with `python3 -m http.server`.

**You are ready when** you have a georeferenced image whose roads sit under the basemap roads, and
an RMS number you can defend.

**The one that bites.** Opening the Leaflet page straight from disk makes the GeoJSON `fetch` fail
silently. Serve the folder over `http://localhost:8000` instead, or test it on GitHub Pages.
