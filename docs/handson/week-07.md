# Week 7 Hands-On — Web Services in QGIS

**Hands-On Practice**{ .badge .badge-handson } · *Day 13 · Thursday of [Week 7 — Finding Spatial Data and Web Services](../weeks/week-07.md)*
...
> [!TIP]
> Twenty minutes, all of it online. Rehearse this one on the network you will actually teach on.

**What you need.** QGIS 3.44 LTR and internet. No downloads at all, which is the lesson.

**Do this, in order.**

1. Open `https://services1.arcgis.com/99lidPhWCzftIe9K/ArcGIS/rest/services/` in a browser and
   scroll the whole listing once so you know what is there. Click into a roads service, then a
   layer, and read its fields, geometry type, spatial reference and record count.
2. On that layer page click **Query**, set `Where` to `1=1`, `Out fields` to `*`, format JSON, and
   run it. That JSON is what QGIS is about to receive.
3. **Layer > Data Source Manager > ArcGIS REST Server > New**, name `UGRC`, paste the endpoint,
   OK, then **Connect**.
4. Add three layers of different geometry: a polygon such as counties or municipalities, a line
   such as roads or streams, and a point layer such as schools or hospitals.
5. Right-click one > **Properties > Source**. Confirm the provider reads `arcgisfeatureserver`.
   Open the **Query Builder** and filter to one county using the real field name, then watch the
   layer redraw with far less data.
6. Add any public WMS through **Data Source Manager > WMS/WMTS**, then try **Identify** on it and
   find no attribute table. That is the WMS trade.
7. Build a quick print layout with the required elements and a data-source label naming the
   services, and export a PNG.

**You are ready when** three service layers draw, your county filter works, and you know from
experience which of those services is slow enough to avoid in class.

**The one that bites.** There is no offline fallback for this session. If the room's connection is
unreliable, move to the wired lab machines in the Clyde building rather than improvising.
