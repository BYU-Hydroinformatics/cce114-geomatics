# Week 2 Hands-On — First Map in QGIS

**Hands-On Practice**{ .badge .badge-handson } · *Day 3 · Thursday of [Week 2 — GIS Data Models](../weeks/week-02.md)*
...
1. New empty project. In the **Browser** panel expand **XYZ Tiles** and double-click
   **OpenStreetMap**. A world basemap appears.
2. Right-click **XYZ Tiles > New Connection...**, name it `Google Satellite`, paste
   `https://mt1.google.com/vt/lyrs=s&x={x}&y={y}&z={z}`, click OK, double-click the new entry.
3. **Layer > Add Layer > Add Vector Layer...** and add the county boundary, the roads and the
   towers. Pick the `.shp` file each time, not the `.dbf` or `.shx`.
4. **Layer > Add Layer > Add Raster Layer...** and add the DEM.
5. For each layer, right-click **Properties > Information** and find the line naming its data
   model: *Geometry: Point*, *Line*, *Polygon*, and for the DEM the *Dimensions* and *Pixel size*.
6. **Layer > Create Layer > New GeoPackage Layer...**, database `week02.gpkg`, table
   `new_towers`, geometry **Point**, CRS **EPSG:26912**, fields `name` (text) and `height_m`
   (integer). Toggle editing, add three points, fill the form, **Save Layer Edits**.
7. Select the roads layer, toggle editing, pick the **Vertex Tool**, drag one vertex, then toggle
   editing off and choose **Discard**. Watch the road snap back.

**You are ready when** the Layers panel holds a basemap, four county layers and your own point
layer, and you can say out loud which data model each one is without opening the dialog.

**The one that bites.** Adding a shapefile gives "Invalid data source" because a sidecar file was
picked instead of the `.shp`. Do it once on purpose so you recognize it in the room.
