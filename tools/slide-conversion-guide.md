# Converting a CCE 114 PowerPoint into a Marp web slide deck

This is the recipe used for Day 1 and Day 2. Follow it exactly so every deck looks and behaves the
same. Read `slides/day-02/gis-data-models.md` first: it is the reference for structure, tone,
and formatting.

## Where things go

- Deck: `slides/day-NN/<slug>.md` (Marp markdown). One deck per file; a day may have two.
...
- The site builds each `slides/*/*.md` with marp-cli on every push and publishes it at
  `https://byu-hydroinformatics.github.io/cce114-geomatics/slides/day-NN/<slug>.html`.
...
for the CCE 114 rhythm: Tuesdays are concept lectures, Thursdays are demos and hands-on work in
QGIS. Keep the source deck's content, order, and voice; tighten wording where the original was
rough.
...
- End with a **Before Next Class** slide: the textbook reading, the open-book quiz on Learning
  ...
  session, add a short **Thursday: hands-on in QGIS** preview slide before it.
...
- **Software wording:** the course now uses QGIS, not ArcGIS/ArcMap/ArcGIS Pro. Change text that
  ...
  class" explained as QGIS layer/GeoPackage, and so on). Leave concepts alone. If a slide is an
  ArcGIS screenshot, keep it for now and flag it (see Step 5); do not fabricate a QGIS screenshot.
...
theme: cce114
paginate: true
footer: "CCE 114 · Day NN — <Deck Title>"
---
```

The theme is `slides/theme/cce114.css` (BYU navy, `lead`/`quiz`/`activity` classes, `columns`,
